-- ============================================================================
-- Digi Dental — RUN THIS ONE FILE. Migration 007 only.
--
-- WHY THIS FILE EXISTS
--   005 and 006 are already applied (site_events has rows and rpc_overview works).
--   Only 007 is missing: rpc_contact, rpc_clicks, rpc_click_totals, rpc_pipeline
--   do not exist in pg_proc. The combined file is 39 KB and its tail did not make
--   it into the editor; this one is 8 KB so that cannot happen again.
--
-- HOW TO RUN
--   Supabase dashboard -> SQL Editor -> New query -> select all of this -> Run.
--   Safe to run twice.
--
-- The last statement prints the four functions. If you see four rows, it worked.
-- ============================================================================
...
where n.nspname = 'public'
  and p.proname in ('rpc_contact','rpc_clicks','rpc_click_totals','rpc_pipeline')
order by 1;
