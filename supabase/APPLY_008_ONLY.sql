-- ============================================================================
-- Digi Dental — RUN THIS ONE FILE. Migration 008 only.
--
-- 005, 006 and 007 are already applied. This adds what the rebuilt dashboard needs:
--   rpc_series             every headline metric per day, for the switchable chart
--   rpc_click_log          individual clicks: when, what, region, device, source
--   rpc_monthly            calendar-month conversions, for the Audit tab
--   rpc_monthly_breakdown  those months split by source / country / device / campaign
--
-- HOW TO RUN
--   Supabase dashboard -> SQL Editor -> New query -> select all -> Run.
--   Safe to run twice.
--
-- The last statement prints the four functions. Four rows means it worked.
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- Proof it worked. Expect exactly four rows.
-- ---------------------------------------------------------------------------
select p.proname as function_created,
       pg_get_function_identity_arguments(p.oid) as arguments
from pg_proc p join pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public'
  and p.proname in ('rpc_series','rpc_click_log','rpc_monthly','rpc_monthly_breakdown')
order by 1;
