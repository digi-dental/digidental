...
-- It is migration 010 on its own. Safe to run more than once.
-- The last statement prints five rows; every `result` should read 1.
--
-- Nothing about the site breaks if you delay this. Events are already recorded with the
-- page they happened on, so no history is lost in the meantime — the dashboard simply
-- cannot split them yet, and the Pages view says so instead of showing zeroes.
-- ============================================================================
...
-- ---------------------------------------------------------------- verification
select '_dd_page created' as check,
       (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
         where n.nspname = 'public' and p.proname = '_dd_page')::text as result
union all
select 'rpc_pages created',
       (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
         where n.nspname = 'public' and p.proname = 'rpc_pages')::text
union all
-- The point of the migration: these two now return a `page` column. 1 means the new shape
-- is live; 0 means the old one is still there and the split will not appear.
select 'rpc_sections is page-aware',
       (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
         where n.nspname = 'public' and p.proname = 'rpc_sections'
           and 'page' = any(p.proargnames))::text
union all
select 'rpc_scroll is page-aware',
       (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
         where n.nspname = 'public' and p.proname = 'rpc_scroll'
           and 'page' = any(p.proargnames))::text
union all
-- Dropping a function drops its grants with it, so this is the one that matters after a
-- re-run: without it /api/stats gets "permission denied for function".
select 'service_role can execute rpc_pages',
       (select has_function_privilege('service_role', 'public.rpc_pages(int)', 'execute'))::int::text;
